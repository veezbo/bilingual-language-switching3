...
if os.path.isfile(trimmedCache) and False: 
	...
	if os.path.isfile(preprocessedCache) and False: 
		...
		chunksTargets_boldDelay="chunksTargets_boldDelay{0}-{1}-LanguageThinking-Japanese_English.txt".format(boldDelay, stimulusWidth)
	...
	print '... and only',dataset.shape[0],'cases of interest (LanguageThink)'
...
# run classifier
print 'learning on detrended, normalised, averaged, LanguageThink ...',datetime.datetime.now()
...
P.savefig(os.path.join(sessionPath,'confMatrixAvTrial{0}-{1}-LanguageThinking-Japanese_English.png'.format(boldDelay, stimulusWidth)))
...
print sens.shape
M.map2nifti(dataset, N.mean(sens,0)).to_filename("anovaSensitivity{0}-{1}-LanguageThink-Japanese_English.nii".format(boldDelay, stimulusWidth))
