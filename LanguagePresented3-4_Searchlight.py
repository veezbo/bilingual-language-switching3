...
if os.path.isfile(trimmedCache) and False: 
	...
	if os.path.isfile(preprocessedCache) and False: 
		...
		chunksTargets_boldDelay='chunksTargets_boldDelay{0}-{1}-Japanese_English.txt'.format(boldDelay, stimulusWidth)
	...
	# dataset = dataset[N.array([l in ['c', 'k'] for l in dataset.sa.targets], dtype='bool')]
	classificationName='Language_Presented-Japanese_English'
