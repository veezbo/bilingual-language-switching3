...
if boldDelayVols>-1:
	pymvpaFileName = 'chunksTargets_boldDelay'+str(boldDelayVols)+'-'+str(stimulusVols)+'-'+'LanguageThinking-'+categoryMarkers.keys()[0]+'_'+categoryMarkers.keys()[1]+'.txt'
else:
...
for line in behavFile:
	...
	if re.search('direction: ', line):
		dirStimulus=re.search('direction: (.+)\n', line).groups(0)[0].strip()
		if dirStimulus == '|' and category==categoryMarkers.values()[0]:
			thinkLang=categoryMarkers.values()[1]
		elif dirStimulus == '|' and category==categoryMarkers.values()[1]:
			thinkLang=categoryMarkers.values()[0]
		elif category==categoryMarkers.values()[0]:
			thinkLang=categoryMarkers.values()[0]
		else:
			thinkLang=categoryMarkers.values()[1]
		...
		pymvpaFile.write(("%s-%s %i\n" % (thinkLang, concept, chunkCounter)) * stimulusVols) # e.g. 3s
		# if direction=='keep':
		# 	pymvpaFile.write(("%s-%s %i\n" % (category+category, concept, chunkCounter)) * stimulusVols) # e.g. 3s
		# elif direction=='switch' and category==categoryMarkers.values()[0]:
		# 	pymvpaFile.write(("%s-%s %i\n" % (category+categoryMarkers.values()[1], concept, chunkCounter)) * stimulusVols) # e.g. 3s
		# else:
		# 	pymvpaFile.write(("%s-%s %i\n" % (category+categoryMarkers.values()[0], concept, chunkCounter)) * stimulusVols) # e.g. 3s
