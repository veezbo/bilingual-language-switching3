...
if boldDelayVols>-1:
	pymvpaFileName = 'chunksTargets_boldDelay'+str(boldDelayVols)+'-'+str(stimulusVols)+'-'+'direction'+'.txt'
else:
...
for line in behavFile:
	...
	if re.search(categoryName,line):
		category=re.search(categoryName+'(.+)\r',line).groups(0)[0] # REMEMBER - to choose \r or \n, depending on Op. System used to produce file	
		if category=='|':
			category='s'
		else:
			category='k'
