...
#Change if you need the targets of classification:this script is for Keep(k) vs Switch(s).
...
P.savefig(os.path.join(sessionPath,'confMatrixAvTrial{0}-{1}-keepswitch.png'.format(boldDelay, stimulusWidth)))
...
print sens.shape
M.map2nifti(dataset, N.mean(sens,0)).to_filename("anovaSensitivity{0}-{1}-keepswitch.nii".format(boldDelay, stimulusWidth))
