...
#Change if you need the targets of classification:this script is for mammal(m) vs tool(t).
...
if os.path.isfile(trimmedCache) and False: 
	...
	if os.path.isfile(preprocessedCache) and False: 
		...
		chunksTargets_boldDelay="chunksTargets_boldDelay{0}-{1}-mammal_tool.txt".format(boldDelay, stimulusWidth)
	...
	dataset = dataset[N.array([l in ['m', 't'] for l in dataset.sa.targets], dtype='bool')]
	print '... and only',dataset.shape[0],'cases of interest (mammals vs tools)'
